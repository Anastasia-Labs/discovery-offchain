...
export const addCollected = async (
  ...
  if (!tokenHolderPolicy?.scriptRef || !tokenHolderValidator?.scriptRef) {
    throw new Error(
      "Could not find the required reference scripts for TokenHolderPolicy and/or TokenHolderValidator.",
    );
  }
